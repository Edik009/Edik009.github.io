#!/usr/bin/env python3
"""
AASFA Scanner v5.0 - Universal Security Scanner
Main entry point

Usage:
  python main.py                    # Interactive Russian menu
  python main.py -t <IP>             # CLI mode (English)
"""
import sys
import argparse
...
def launch_interactive_mode():
    """Launch interactive Russian menu"""
    try:
        from aasfa.cli.interactive_menu import launch_interactive_menu
        launch_interactive_menu()
    except ImportError as e:
        print(f"[!] Error loading interactive menu: {e}")
        print("[!] Falling back to CLI mode (use -t <IP>)")
        sys.exit(1)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="AASFA Scanner v5.0 - Universal Security Scanner",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python main.py                    # Interactive Russian menu (default)
  python main.py -t 192.168.1.100   # CLI mode (English)
  python main.py -t 192.168.1.100 -m deep -o report.txt
  python main.py -t 192.168.1.100 -m deep --threads 20
  python main.py -t 192.168.1.100 -m fast -v
...
Remote analysis only, no USB/ADB required.
    ...
    parser.add_argument(
        '-t', '--target',
        required=False,  # Changed to False for interactive mode
        help='Target device IP address (required for CLI mode, optional for interactive)'
    )
...
def main():
    ...
    if len(sys.argv) == 1:
        # No arguments - launch interactive Russian menu
    ...
    # If no target specified in CLI mode, show help
    if not args.target:
        print("Error: Target IP address is required for CLI mode")
        print("Use: python main.py -t <IP>")
        print("Or run without arguments for interactive menu: python main.py")
        parser = argparse.ArgumentParser()
